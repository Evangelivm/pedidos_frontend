...
import { CreditCard, DollarSign, Smartphone, CheckCircle, User } from "lucide-react"
...
export function PaymentModal({ order, isOpen, onClose, onPaymentComplete }: PaymentModalProps) {
  ...
  const clientsWithRUC = clients.filter((client) => client.ruc && client.ruc.trim() !== "")
  const clientsWithoutRUC = clients.filter((client) => !client.ruc || client.ruc.trim() === "")
  ...
  const handleCompletePayment = () => {
    ...
    if (totalPaid < order.total) {
      toast({
        ...
        description: "El monto total pagado debe ser igual o mayor al total del pedido.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        ...
        description: `El pago del pedido #${order.id.slice(0, 8)} ha sido registrado correctamente.`,
      })
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3 py-1">
          <div className="bg-gray-50 p-2 rounded-md text-sm grid grid-cols-2 gap-1">
            ...
            <div className="col-span-2 flex justify-between">
              ...
              <span className="font-bold text-blue-700">S/.{order?.total.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <div>
              ...
              <Select value={paymentInfo.clientId} onValueChange={handleClientSelect}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Seleccionar cliente" />
                </SelectTrigger>
                <SelectContent>
                  {clients.length === 0 ? (
                    <div className="py-2 text-center text-sm text-gray-500">No hay clientes</div>
                  ) : (
                    <>
                      {clientsWithRUC.length > 0 && (
                        <SelectGroup>
                          ...
                          {clientsWithRUC.map((client) => (
                            <SelectItem key={client.id} value={client.id} className="py-1.5 pl-6 pr-2 text-sm">
                              <div className="flex flex-col">
                                <span className="font-medium">{client.name}</span>
                                <span className="text-xs text-blue-600">RUC: {client.ruc}</span>
                              </div>
                            </SelectItem>
                          ))}
                        </SelectGroup>
          ...
          <div className="border-t pt-2 mt-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              <div>
                <Label htmlFor="cash" className="flex items-center gap-1 text-sm mb-1">
              ...
              <div>
                <Label htmlFor="yape" className="flex items-center gap-1 text-sm mb-1">
              ...
              <div>
                <Label htmlFor="transfer" className="flex items-center gap-1 text-sm mb-1">
                  <CreditCard className="h-3.5 w-3.5 text-blue-600" /> Transferencia
                </Label>
                <div className="relative">
                  <span className="absolute left-2 top-1/2 transform -translate-y-1/2 text-gray-500 text-sm">S/.</span>
                  <Input
          ...
          <div className="bg-gray-50 p-2 rounded-md space-y-1 mt-2">
            <div className="flex justify-between text-base font-bold">
              ...
              <span className={totalPaid >= order?.total ? "text-green-600" : "text-red-600"}>
            ...
            {totalPaid > order?.total && (
              <div className="flex justify-between text-xs">
                <span>Vuelto:</span>
                <span className="text-orange-600 font-medium">S/.{change.toFixed(2)}</span>
              </div>
            )}

            {totalPaid < order?.total && (
              <div className="text-red-500 text-xs">Falta S/.{(order?.total - totalPaid).toFixed(2)}</div>
            )}

            {totalPaid === order?.total && <div className="text-green-600 text-xs font-medium">¡Monto exacto!</div>}
          </div>
        </div>
        ...
        <DialogFooter className="sticky bottom-0 bg-white pt-2 z-10">
          ...
          <Button
            ...
            className={`${
              totalPaid >= order?.total ? "bg-green-600 hover:bg-green-700" : "bg-gray-400 cursor-not-allowed"
            }`}
