...
import { ThermalPrinter, detectPrinters, getDefaultPrinter, setDefaultPrinter } from "@/lib/thermal-printer"
...
export function PrintReceiptModal({ order, isOpen, onClose }: PrintReceiptModalProps) {
  const receiptRef = useRef<HTMLDivElement>(null)
  const { toast } = useToast()
  const [selectedItems, setSelectedItems] = useState<number[]>(order?.items.map((item: any) => item.id) || [])
  ...
  const handlePrint = async () => {
    try {
      if (receiptRef.current) {
        // Crear un iframe para impresión controlada que preserve exactamente el formato
        ...
        // Filtrar order items basado en selección
        const filteredOrder = {
          ...order,
          items: order.items.filter((item: any) => selectedItems.includes(item.id)),
          total: calculateTotal(order.items.filter((item: any) => selectedItems.includes(item.id))),
        }

        // Intentar imprimir usando la biblioteca de impresión térmica
        const success = await ThermalPrinter.printReceipt(filteredOrder, { showDialog: true })
  ...
  // Filter order items based on selection
  const filteredOrder = {
    ...order,
    items: order.items.filter((item: any) => selectedItems.includes(item.id)),
    total: calculateTotal(order.items.filter((item: any) => selectedItems.includes(item.id))),
  ...
  return (
    <>
        ...
        <DialogContent className={showPreview ? "max-w-md" : "max-w-2xl"}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {showPreview ? "Vista previa de impresión" : "Seleccionar productos para imprimir"}
          ...
          {showPrinterSettings && !showPreview ? (
            <div className="space-y-4 py-2">
              <div className="bg-blue-50 p-3 rounded-md text-sm text-blue-700 mb-4">
                <p className="font-medium mb-1">Configuración de impresora térmica</p>
                <p>Seleccione la impresora térmica que desea utilizar para imprimir recibos.</p>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <Button size="sm" onClick={handleSetDefaultPrinter} disabled={!selectedPrinter}>
                  Establecer como predeterminada
                </Button>
              </div>

              <div className="pt-4 flex justify-end">
                <Button onClick={() => setShowPrinterSettings(false)}>Continuar</Button>
              </div>
            </div>
          ) : !showPreview ? (
            <>
              <div className="mb-2 flex items-center">
                <Checkbox
                  id="select-all"
                  checked={selectedItems.length === order.items.length && order.items.length > 0}
              ...
              <div className="flex justify-between items-center mt-4">
                <div>
                  <p className="text-sm text-gray-500">
                    {selectedItems.length} de {order.items.length} productos seleccionados
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold">
                    Total:{" "}
                    {formatCurrency(calculateTotal(order.items.filter((item: any) => selectedItems.includes(item.id))))}
                  </p>
                </div>
              </div>
            ...
            <>
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                {hasPrinted ? (
                  <Button onClick={() => setShowDeliveryModal(true)} className="bg-green-600 hover:bg-green-700">
      ...
      <DeliveryInfoModal orderId={order.id} isOpen={showDeliveryModal} onClose={() => setShowDeliveryModal(false)} />
    </>
  )
}
